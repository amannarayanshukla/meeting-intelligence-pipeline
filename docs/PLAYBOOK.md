...
## 8. Load test
    ./demo.sh load                                   # 100 concurrent submits against the running stack, runs inside Docker (no Node needed)
    ./demo.sh load 20                                # smaller burst
    WORKER_CONCURRENCY=30 ./demo.sh && ./demo.sh load   # restart with 10× the workers, then the same burst
